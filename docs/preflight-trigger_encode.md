...
Encode a value or local file; value or file location is required

### Synopsis

Encode accepts either a value or a local file to encode and print to stdout.
If you set the output-path the encoded data will be written to the specified file.

```
preflight-trigger encode [flags]
```

### Options

```
      --file string    Path to file to encode
